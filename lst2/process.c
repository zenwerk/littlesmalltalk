...
void prpush(newobj) object newobj;
{
...
static object findMethod(hash, message, startingClass) int hash;
...
via side effects
*/
static void newContext(method, methodClass, contextobj, argobj,
                       tempobj) object method,
  methodClass, *contextobj, argobj, *tempobj;
{
  int temporarysize;
...
void sendMessage(message, startingClass, argumentPosition) object message,
  startingClass;
int argumentPosition;
{
   ...
   * charPtr(basicAt(startingClass, nameInClass)));*/
    ...
    while (!done)
    {
      ...
      switch (finalTask)
      {
        case sendMessageTask:
          sendMessage(messageToSend, getClass(processStack[argumentsOnStack]),
                      argumentsOnStack);
        ...
        case ContextExecuteTask:
          ...
          executeFromContext(contextobj, argumentsOnStack);
          decr(contextobj);
...
*/
void executeFromContext(context, bytecounter) object context;
int bytecounter;
{
  ...
  while (!done)
  {
    ...
    switch (finalTask)
    {
      ...
      case sendSuperTask:
        sendMessage(messageToSend, basicAt(methodclass, superClassInClass),
                    argumentsOnStack);
        if (finalTask == BlockReturnTask)
          done = true;
        break;
...
static void interpush(interp, value) object interp, value;
{
  int stacktop;
  object stack;
...
object doInterp(interpreter) object interpreter;
{
  ...
  execute(method, bytecounter, memoryPtr(stack), stacktop, memoryPtr(arguments),
          memoryPtr(temporaries));
  basicAtPut(interpreter, stackTopInInterpreter, newInteger(finalStackTop));
  basicAtPut(interpreter, byteCodePointerInInterpreter,
             newInteger(finalByteCounter));
  ...
  switch (finalTask)
  {
    ...
    case sendSuperTask:
      ...
      if (finalTask == sendMessageTask)
        ...
        class =
          basicAt(basicAt(context, methodClassInContext), superClassInClass);
      ...
      newContext(method, methodCache[hash].methodClass, &contextobj, argobj,
                 &tempobj);
      basicAtPut(interpreter, stackTopInInterpreter, newInteger(finalStackTop));
      argumentsOnStack = 0;
    ...
    case ContextExecuteTask:
      ...
      basicAtPut(newinterp, byteCodePointerInInterpreter,
                 newInteger(argumentsOnStack));
      decr(contextobj);
      return (newinterp);
      break;
