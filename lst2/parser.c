...
void setInstanceVariables(aClass) object aClass;
{
  int i, limit;
  object vars;
...
void compilError(str1, str2) char *str1, *str2;
{
...
int genLiteral(aLiteral) object aLiteral;
{
...
int parseArray()
{
  ...
  while (parseok && (token != closing))
  {
    switch (token)
    {
      ...
      case charconst:
        ignore genLiteral(newChar(newInteger(tokenInteger)));
        ignore nextToken();
        break;
      ...
      default:
        compilError("illegal text in literal array", tokenString);
        ignore nextToken();
        break;
    }
  }
  ...
  {
    if (!streq(tokenString, ")"))
      compilError("array not terminated by right parenthesis", tokenString);
    else
      ignore nextToken();
  }
...
boolean term()
{
  ...
  else if (token == intconst)
  {
    if ((tokenInteger >= 0) && (tokenInteger <= 2))
      ...
      genInstruction(PushLiteral, genLiteral(newInteger(tokenInteger)));
    ignore nextToken();
  }
  ...
  else if ((token == binary) && streq(tokenString, "-"))
  {
    if (nextToken() != intconst)
      compilError("negation not followed", "by integer");
...
void genMessage(toSuper, argumentCount, messagesym) boolean toSuper;
int argumentCount;
object messagesym;
{
  if (toSuper)
...
boolean unaryContinuation(superReceiver) boolean superReceiver;
{
  int i;
  boolean sent;
  object messagesym;
...
boolean binaryContinuation(superReceiver) boolean superReceiver;
{
  int i;
...
boolean keyContinuation(superReceiver) boolean superReceiver;
{
  ...
  if (token == namecolon)
  {
    ...
    else if (streq(tokenString, "whileTrue:"))
    {
      ...
      for (j = codeTop - 1; j > 0; j--)
        if ((codeArray[j] == savetop) && (codeArray[j - 1] == CreateBlock * 16))
        {
          genCode(j - 1);
          break;
        }
...
void continuation(superReceiver) boolean superReceiver;
{
...
void statement()
{
  ...
  { /* possible assignment */
    ...
    if ((token == binary) && streq(tokenString, "<-"))
    {
      ...
      if (inBlock && (token == closing) && streq(tokenString, "]"))
        genInstruction(DoSpecial, StackReturn);
    }
    else
...
void block()
{
  ...
  if ((token == binary) && streq(tokenString, ":"))
  {
    ...
    {
      if (nextToken() != name)
        compilError("name must follow colon", "in block argument list");
    ...
    if ((token != binary) || !streq(tokenString, "|"))
      compilError("block argument list must be terminated", "by |");
    ignore nextToken();
  }
...
void messagePattern()
{
  ...
  { /* keyword message pattern */
    ...
    while (parseok && (token == namecolon))
    {
      ...
      ignore nextToken();
      if (token != name)
        compilError("keyword message pattern", "not followed by a name");
...
boolean parse(method, text) object method;
char *text;
{
  int i;
