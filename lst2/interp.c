...
#define ipop(x)          \
  x = stack[--stacktop]; \
  stack[stacktop] = nilobj

void execute(method, byteCounter, stack, stacktop, arguments,
             temporaries) object method,
  *stack, *arguments, *temporaries;
...
register int stacktop;
{
  ...
  while (!done)
  {
    ...
    switch (high)
    {
      ...
      case PushGlobal:
        newobj = nameTableLookup(globalNames, literals[low]);
        if (newobj == nilobj)
        {
          ...
          messageToSend = newSymbol("cantFindGlobal:");
      ...
      case SendBinary:
        ...
        if (low <= 12)
        {
          if (isInteger(stack[stacktop - 1]) && isInteger(stack[stacktop - 2]))
          {
            ipop(newobj);
            i = intValue(newobj);
            ipop(newobj);
          ...
          if (isFloat(stack[stacktop - 1]) && isFloat(stack[stacktop - 2]))
          {
            ipop(newobj);
            f = floatValue(newobj);
            decr(newobj);
      ...
      case CreateBlock:
        ...
        i = (low > 0) ? nextByte : 0;
        basicAtPut(newobj, argumentLocationInBlock, newInteger(i));
      ...
      case DoSpecial:
        switch (low)
        {
          ...
          case SendToSuper:
            argumentsOnStack = stacktop - (nextByte + 1);
            messageToSend = literals[nextByte];
