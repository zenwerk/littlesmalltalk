...
/* global variables used to avoid repeated examinations of the global symbol
 * table */
...
char *unStrs[] = {"isNil", "notNil",    "new",   "value",       "class",
                  "size",  "basicSize", "print", "printString", 0};

object unSyms[9];

char *keyStrs[] = {
  "at:ifAbsent:", "at:put:", "basicAt:put:", "between:and:", 0};

object keySyms[4];

object nameTableLookup(table, symbol) object table, symbol;
{
  int hash, tablesize;
  object link;
...
object getClass(obj) object obj;
{
  if (isInteger(obj))
    return (intclass);
...
object globalGet(name) char *name;
{
  object newobj;
...
object newSymbol(str) char *str;
{
  ...
  for (link = basicAt(symbols, hash + 2); link != nilobj;
       link = basicAt(link, 2))
  {
