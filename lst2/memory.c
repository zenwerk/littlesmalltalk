...
void sysError(s1, s2) char *s1, *s2;
{
  int i;
  fprintf(stderr, "%s\n%s\n", s1, s2);
  i = 0;
...
object alcObject(memorySize, memoryType) int memorySize;
int memoryType;
{
  int position;
  int trip;
  ...
  { /* not found, must allocate a new object */
    position = trip = 0;
    do
    {
...
object allocSymbol(str) char *str;
{
  object newSym;
...
void basicAtPut(z, i, v) object z, v;
register int i;
{
  if (isInteger(z))
...
void iwerr()
{
  sysError("imageWrite count error", "");
}

void imageWrite(fp) FILE *fp;
{
  short i;

  if (fwrite(&symbols, sizeof(object), 1, fp) != 1)
    iwerr();
  if (fwrite(&globalNames, sizeof(object), 1, fp) != 1)
    iwerr();
  ...
  for (i = 0; i < ObjectTableMax; i++)
  {
    ...
    {
      if (fwrite(&i, sizeof(short), 1, fp) != 1)
        iwerr();
      if (fwrite(&objectTable[i].class, sizeof(object), 1, fp) != 1)
        iwerr();
      if (fwrite(&objectTable[i].size, sizeof(byte), 1, fp) != 1)
        iwerr();
      if (fwrite(&objectTable[i].type, sizeof(byte), 1, fp) != 1)
        iwerr();
      if (objectTable[i].size != 0)
        if (fwrite(objectTable[i].memory, sizeof(object), objectTable[i].size,
                   fp) != objectTable[i].size)
          iwerr();
    }
  }
}
...
void imageRead(fp) FILE *fp;
{
  short i;
  object *p;

  if (fread(&symbols, sizeof(object), 1, fp) != 1)
    irerr();
  if (fread(&globalNames, sizeof(object), 1, fp) != 1)
    irerr();
  ...
  {
    if (fread(&objectTable[i].class, sizeof(object), 1, fp) != 1)
      irerr();

    if (fread(&objectTable[i].size, sizeof(byte), 1, fp) != 1)
      irerr();
    if (fread(&objectTable[i].type, sizeof(byte), 1, fp) != 1)
      irerr();
    if (objectTable[i].size != 0)
    {
      p = objectTable[i].memory = mBlockAlloc((int)objectTable[i].size);
      if (fread(p, sizeof(object), byteToInt(objectTable[i].size), fp) !=
          byteToInt(objectTable[i].size))
        irerr();
      if (objectTable[i].referenceCount > 0)
        membump(i, i);
    }
...
double floatValue(obj) object obj;
{
  double d;
