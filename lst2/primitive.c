...
static int zeroaryPrims(number) int number;
{
  char buffer[100];
  short i;
...
int intBinary(number, firstarg, secondarg) register int firstarg, secondarg;
int number;
{
  boolean binresult = 0;
...
int floatBinary(number, first, second) int number;
double first, second;
{
  boolean binResult = 0;
...
boolean primitive(primitiveNumber, arguments, size) int primitiveNumber, size;
object *arguments;
{
  int primitiveGroup;
  boolean done = false;
  ...
  switch (primitiveGroup)
  {
    ...
    case 3:
      ...
      else
      {
        switch (primitiveGroup)
        {
          ...
          case 2:
            response =
              binaryPrims(primitiveNumber - 20, arguments[0], arguments[1]);
            break;
          case 3:
            response = trinaryPrims(primitiveNumber - 30, arguments[0],
    ...
    case 5: /* integer unary operations */
      if (size != 1)
        ...
        response = intUnary(primitiveNumber - 50, intValue(arguments[0]));
      break;
    ...
    case 7: /* integer binary operations */
      if (size != 2)
        response = counterror;
      else if ((!isInteger(arguments[0])) || !isInteger(arguments[1]))
        response = typeerror;
      else
        response = intBinary(primitiveNumber - 60, intValue(arguments[0]),
    ...
    case 8: /* string unary */
      if (size != 1)
        ...
        response = strUnary(primitiveNumber - 80, charPtr(arguments[0]));
      break;
    ...
    case 10: /* float unary */
      if (size != 1)
        ...
        response = floatUnary(primitiveNumber - 100, floatValue(arguments[0]));
      break;

    case 11: /* float binary */
      if (size != 2)
        response = counterror;
      else if ((!isFloat(arguments[0])) || (!isFloat(arguments[1])))
        response = typeerror;
      else
        response = floatBinary(primitiveNumber - 110, floatValue(arguments[0]),
