...
genclass(clinfo, mlist) struct classstruct *clinfo;
struct methodstruct *mlist;
{
  ...
  for (m = mlist, i = 0; m; i++, m = m->nextmethod)
    for (n = m->nextmethod; n; n = n->nextmethod)
      if (streq((m->pattern)->cmdname, (n->pattern)->cmdname))
        yerr("%s multiply defined", (m->pattern)->cmdname);
...
static gencode(value) int value;
{
  if (value >= 256)
  {
...
static genhighlow(high, low) int high, low;
{
  if (high < 0 || high > 16)
...
int litcomp(l1, l2) struct litstruct *l1, *l2;
{
  if (l1->littype != l2->littype)
    return (0);
  switch (l1->littype)
  {
    case charlit:
      if (l1->ll.litchar != l2->ll.litchar)
        return (0);
      break;
    case numlit:
      if (l1->ll.litint != l2->ll.litint)
        return (0);
      break;
    case fnumlit:
      if (l1->ll.litstr != l2->ll.litstr)
        return (0);
      break;
    case strlit:
      if (strcmp(l1->ll.litstr, l2->ll.litstr))
        return (0);
      break;
    case symlit:
      if (strcmp(l1->ll.litsym, l2->ll.litsym))
        return (0);
      break;
    default:
      return (0);
...
int genlitix(l) struct litstruct *l;
{
  int i;
...
static printalit(lit) struct litlist *lit;
{
  if (lit)
  {
    if (lit->nextlit)
...
printlit(lit) struct litstruct *lit;
{
  if (lit)
    switch (lit->littype)
    {
      case numlit:
        fprintf(ofd, "%d ", lit->ll.litint);
        break;
      case fnumlit:
        fprintf(ofd, "%s ", lit->ll.litstr);
        break;
      case charlit:
        fprintf(ofd, "$%c ", lit->ll.litchar);
        break;
      case strlit:
        fprintf(ofd, "\'%s\' ", lit->ll.litstr);
        break;
      case symlit:
        fprintf(ofd, "#%s ", lit->ll.litsym);
        break;
      case arlit:
        fprintf(ofd, "#( ");
        ...
        break;
      default:
        yerr("unknown literal type %d", lit->littype);
    }
}

genmeth(m) struct methodstruct *m;
{
  int i;
...
genstate(s, doret) struct statestruct *s;
int doret;
{
  if (s->nextstate)
...
gensexpr(s) struct statestruct *s;
{
  switch (s->statetype)
  {
    default:
...
int supertest(rec) struct exprstruct *rec;
{
  struct objstruct *o;
...
int isblock(e) struct exprstruct *e;
{
  struct objstruct *o;
...
int gencond(message, e) char *message;
struct exprstruct *e;
{
  struct keylist *k;
  int i, j;
  ...
  if ((j = streq(message, "whileTrue:")) || streq(message, "whileFalse:"))
  {
    i = codetop;
    genbarg(e->receiver);
...
int genexpr(e) struct exprstruct *e;
{
  ...
  switch (e->cmdtype)
  {
    ...
    case keycmd:
      if ((!s) && isblock(e->receiver) &&
          (streq(message, "whileTrue:") || streq(message, "whileFalse:")))
        if (gencond(message, e))
          return 0;
      genexpr(e->receiver);
      if ((!s) &&
          ((streq(message, "ifTrue:")) || (streq(message, "ifFalse:")) ||
           (streq(message, "and:")) || (streq(message, "or:")) ||
...
int genkargs(kl) struct keylist *kl;
{
  int i;
...
int genspclass(litinfo) struct litstruct *litinfo;
{
  int i;
  char **p, *name;
...
genpseu(p) enum pseuvars p;
{

  switch (p)
  {
    default:
      yerr("unknown state in genpseu %d", p);
    case truevar:
      genhighlow(PUSHSPECIAL, 11);
      break;
    case falsevar:
      genhighlow(PUSHSPECIAL, 12);
      break;
    case nilvar:
      genhighlow(PUSHSPECIAL, 13);
      break;
    case smallvar:
      genhighlow(PUSHSPECIAL, 14);
      break;
    case procvar:
      genhighlow(PUSHSPECIAL, 15);
      break;
    case supervar:
    case selfvar:
      genhighlow(PUSHTEMP, 0);
      break;
  }
  bumpstk();
}

int genpargs(p) struct primlist *p;
{
  int i;
...
genblock(b) struct blockstruct *b;
{
  int i, size, bsave;
