...
struct classstruct *mkclass(classname, supername) char *classname, *supername;
{
...
struct varstruct *mkvar(text, vtype) char *text;
enum vartypes vtype;
{
  struct varstruct *p;
...
struct varstruct *addvlist(varnode, vlist) struct varstruct *varnode, *vlist;
{
  varnode->nextvar = vlist;
  if (vlist)
    varnode->position = 1 + vlist->position;
  else
    varnode->position = 0;
  return (varnode);
}

addtemp(name, vtype) char *name;
enum vartypes vtype;
{
...
struct varstruct *invlist(varnode, name) struct varstruct *varnode;
char *name;
{
...
struct methodstruct *mkmethod(pat, temps, state) struct exprstruct *pat;
int temps;
...
struct statestruct *mkstate(type, name, sexpr) enum statetypes type;
char *name;
...
struct exprstruct *mkexpr(receiver, type, name,
                          args) struct exprstruct *receiver,
  *args;
enum cmdtypes type;
char *name;
{
...
struct keylist *mkklist(kl, kw, ka) struct keylist *kl;
char *kw;
...
mkkname(kl, kb) struct keylist *kl;
char *kb;
{
  if (kl->nextkey)
...
struct exprstruct *mkkey(receiver, keywordlist) struct exprstruct *receiver;
...
struct objstruct *mkobj(type, info) enum objtypes type;
YYSTYPE *info;
{
...
struct blockstruct *mkblock(numargs, bstates) int numargs;
...
struct primstruct *mkprim(pnum, plist) struct primlist *plist;
int pnum;
{
...
struct primlist *addprim(plist, prim) struct primlist *plist;
...
struct litlist *addlit(list, lit) struct litlist *list;
...
struct litstruct *mklit(littype, e) enum littypes littype;
YYSTYPE *e;
{
  struct litstruct *p;
