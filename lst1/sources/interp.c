...
interpreter *cr_interpreter(sender, receiver, literals, bitearray,
                            context) interpreter *sender;
...
free_terpreter(anInterpreter) interpreter *anInterpreter;
{
...
copy_arguments(anInterpreter, argLocation, argCount,
               argArray) interpreter *anInterpreter;
...
#define push(x)                            \
  {                                        \
    assign(*(anInterpreter->stacktop), x); \
    anInterpreter->stacktop++;             \
  }

/* push_object - push a returned value on to an interpreter stack */
push_object(anInterpreter, anObject) interpreter *anInterpreter;
object *anObject;
{
...
#define nextbyte(x)                         \
  {                                         \
    x = uctoi(*anInterpreter->currentbyte); \
...
int resume(anInterpreter) register interpreter *anInterpreter;
{
  int highBits;
  ...
  switchtop:
    ...
    switch (highBits)
    {
      ...
      case 5: /* special literals */
        ...
        {
          /* get class */
          tempobj = new_sym(classpecial[lowBits - 30]);
          tempobj = primitive(FINDCLASS, 1, &tempobj);
        }
        else
          tempobj = new_int(lowBits);
      ...
      case 9: /* send a superclass message */
        ...
        if (!is_symbol(tempobj))
          cant_happen(9);
        message = symbol_value(tempobj);
        receiver = fnd_super(anInterpreter->receiver);
        goto do_send2;
      ...
      case 14: /* block creation */
        ...
        push(new_block(anInterpreter, numargs, arglocation));
        skip(i);
        break;
      ...
      case 15: /* special bytecodes */
        ...
        switch (lowBits)
        {
          ...
          case 10: /* execute a primitive */
            ...
            {
              decstack(numargs);
              tempobj = primitive(i, numargs, anInterpreter->stacktop);
              push(tempobj);
            }
            break;
...
do_send2:
  decstack(numargs + 1);
  send_mess(anInterpreter, receiver, message, anInterpreter->stacktop, numargs);
  return 0;
...
blk_execute:
  ...
  sender = block_execute(anInterpreter->sender, (block *)tempvar(0), numargs,
                         &tempvar(1));
  link_to_process(sender);
  return 0;
}
