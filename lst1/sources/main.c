...
char **argv;
{
  ...
  { /* gotta do it the hard way */
    ...
    sassign(o_drive,
            (object *)cr_interpreter((interpreter *)0, null_object, null_object,
...
class *null_class(name) char *name;
{
  class *new, *new_class();
...
char c = 'a';
gettemp(buffer) char *buffer;
{
  sprintf(buffer, "%s%c", filebase, c++);
  if (c > 'z')
    c = 'a'; /* wrap around forever */
}
