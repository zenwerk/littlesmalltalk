...
class *mk_class(classname, args) char *classname;
object **args;
{
  class *new;
  object *new_iarray();
  ...
  else if (streq(classname, "ArrayedCollection"))
  {
    ...
    assign(o_false, new_inst(new)) else if (streq(classname, "Magnitude"))
      assign(o_magnitude, new_inst(new)) else if (streq(classname, "Number"))
        assign(o_number, new_inst(new)) else if (streq(classname, "Object"))
          assign(o_object,
                 new_inst(new)) else if (streq(classname, "Smalltalk"))
            assign(o_smalltalk,
                   new_inst(new)) else if (streq(classname, "True"))
              assign(o_true, new_inst(new)) else if (streq(classname,
                                                           "UndefinedObject"))
                assign(o_nil, new_inst(new)) return (new);
}

/* new_sinst - new instance with explicit super object */
object *new_sinst(aclass, super) class *aclass;
object *super;
{
  object *new;
  ...
  if (streq(classname, "Block") || streq(classname, "Char") ||
      streq(classname, "Class") || streq(classname, "Float") ||
      streq(classname, "Integer") || streq(classname, "Process") ||
...
object *new_inst(aclass) class *aclass;
{
  ...
  {
    super_class = (class *)lookup_class(symbol_value(sp_class_name));
