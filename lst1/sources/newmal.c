  ...
  register int nu;   /* size index to get more of  */
{
  ...
  rnu =
    (nu <= 8) ? 11 : nu + 3; /* take 2k unless the block is bigger than that */
...
free(ap) register unsigned char *ap;
{
  register int si;
...
char *realloc(p, nbytes) register char *p;
register unsigned nbytes;
{
  register char *res;
