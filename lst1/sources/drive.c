...
gencode(value) register int value;
{
  if (value >= 256)
...
genhighlow(high, low) register int high;
register int low;
{
  if (high < 0 || high > 16)
...
int isbinary(bbuf) char *bbuf;
{
  if (token == BINARY || token == MINUS || token == BAR || token == PE)
  {
    strcpy(bbuf, t.c);
    nextlex();
    if (token == BINARY || token == MINUS || token == BAR || token == PE)
...
int primary(must) int must; /* must we find something ? */
{
  int i, count;
    ...
    case PSEUDO:
      switch (t.p)
      {
        ...
        case smallvar:
          ...
          break;
        default:
          lexerr("unknown pseudo var %d", t.p);
      }
...
int aliteral(must) int must; /* must we find something ? */
{
  char *c;
  object *new;
  int count;
...
int gensend(message, numargs) char *message;
int numargs;
{
  int i;
  char **p, c;
...
int bld_interpreter()
{
  ...
  interp = cr_interpreter((interpreter *)o_drive, var_values, literals,
                          bytecodes, context);
