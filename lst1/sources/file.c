...
free_file(phil) struct file_struct *phil;
{
  if (!is_file(phil))
    cant_happen(8);
  if (phil->fp != NULL)
...
file_open(phil, name, type) struct file_struct *phil;
char *name, *type;
{
  char buffer[100];
...
object *file_read(phil) struct file_struct *phil;
{
  object *new;
  int c;
...
void file_write(phil, obj) struct file_struct *phil;
object *obj;
{
  if (phil->fp == NULL)
  {
