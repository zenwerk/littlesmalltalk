...
process *cr_process(anInterpreter) interpreter *anInterpreter;
{
  process *new;
...
free_process(aProcess) process *aProcess;
{
...
link_to_process(anInterpreter) interpreter *anInterpreter;
{
  object *temp;
...
remove_process(aProcess) process *aProcess;
{
...
int set_state(aProcess, state) process *aProcess;
int state;
{
  switch (state)
  {
    ...
    case READY:
    case UNBLOCKED:
      if ((aProcess->p_state ^ state) == ~ACTIVE)
