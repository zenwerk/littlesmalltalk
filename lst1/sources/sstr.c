  ...
  for (; (c = getc(fd)) != EOF;)
    ...
    else
    {
      ...
      *p = '\0';
      p = w_search(buffer, 0);
      if (p)
        printf("&x_str[%d]", p - &x_str[0]);
      else
        printf("\"%s\"", buffer);
    }
}

maketab(ifd, ofd, itab) FILE *ifd, *ofd;
char *itab;
{
  char wbuf[100], *p;
  int i;
  ...
  if (itab)
  {
    ...
    for (i = 0; i <= x_tmax; i++)
    {
      fprintf(ofd, "{1, SYMBOLSIZE, &x_str[%d]}, /* ", x_tab[i] - x_tab[0]);
      for (p = x_tab[i]; *p; p++)
        putc(*p, ofd);
      fprintf(ofd, " */\n");
    }
...
char *w_search(word, insert) char *word;
int insert;
{
  int i, j, k;
