...
static interpreter *cpyInterpreter(anInterpreter) interpreter *anInterpreter;
{
  interpreter *new;

  new = cr_interpreter((interpreter *)0, anInterpreter->receiver,
                       anInterpreter->literals, anInterpreter->bytecodes,
...
object *new_block(anInterpreter, argcount,
                  arglocation) interpreter *anInterpreter;
...
interpreter *block_execute(sender, aBlock, numargs, args) interpreter *sender;
block *aBlock;
int numargs;
object **args;
{
  ...
  {
    sassign(tempobj, new_str("wrong number of arguments for block"));
  ...
  if (numargs)
    copy_arguments(newInt, aBlock->b_arglocation, numargs, args);
  return (newInt);
}

/* block_return - return an object from the context in which a block was
created */
int block_return(blockInterpreter, anObject) interpreter *blockInterpreter;
object *anObject;
{
