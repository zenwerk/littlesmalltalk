...
int send_mess(sender, receiver, message, args, numargs) interpreter *sender;
...
int numargs;
{
  ...
  for (robject = receiver; robject;)
  {
    ...
    if ((objclass == (class *)0) || !is_class(objclass))
      break;

    message_array = objclass->message_names;
    for (i = 0; i < message_array->size; i++)
    {
      if (symbol_value(message_array->inst_var[i]) == message)
      {
...
int responds_to(message, aClass) char *message;
class *aClass;
{
  ...
  for (i = 0; i < message_names->size; i++)
    if (streq(symbol_value(message_names->inst_var[i]), message))
      return (1);
  return (0);
}

/* backtrace - generate a backwards message passing trace */
backtrace(current) interpreter *current;
{
  while (is_interpreter(current->sender) && !is_driver(current->sender))
  {
...
int fnd_message(receiver, bytecodes) object *receiver, *bytecodes;
{
  int i;
  class *oclass;
  object *messar, *temp;
  ...
  for (i = 0; i < messar->size; i++)
  {
    if ((messar->inst_var[i])->inst_var[0] == bytecodes)
    {
      sprintf(buffer, "%s: backtrace. message  %s",
              ...
              symbol_value((oclass->message_names)->inst_var[i]));
...
/* prnt_messages - print all the messages a class responds to.
    ...
    do: */
prnt_messages(aClass) class *aClass;
{
  object *message_names;
  int i;
