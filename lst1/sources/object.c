...
char *o_alloc(n) unsigned n;
{
  char *p;

  p = malloc(n);
  if (p == (char *)0)
    cant_happen(1); /* out of memory */
  n_mallocs++;
  return (p);
}
  ...
  void ob_dec(x) object *x;
  {
    ...
    { /* free a built-in object */
      switch (x->size)
      {
        ...
        case INTEGERSIZE:
        case CHARSIZE:
          free_integer((integer *)x);
          break;
        case INTERPSIZE:
          free_terpreter((interpreter *)x);
          break;
        case PROCSIZE:
          free_process((process *)x);
          break;
        case SYMBOLSIZE:
          cant_happen(16);
        case STRINGSIZE:
          free_string((string *)x);
          break;
        default:
          cant_happen(6);
      }
    ...
    { /* free a normal (non-special) object */
      if (x->super_obj)
        obj_dec(x->super_obj);
      free_obj(x, 1);
    }
...
#define sizeobj(x) (sizeof(object) + ((x)-1) * sizeof(object *))
  ...
  int ca_cobj[5] = {0, 0, 0, 0,
                    0}; /* count how many allocations for small vals*/
  ...
  object *new_obj(nclass, nsize, alloc) class *nclass;
  int nsize, alloc;
  {
    register object *new;
    int i;
  ...
  free_obj(obj, dofree) register object *obj;
  int dofree;
  {
    int size, i;
