...
static int messTest(obj) object obj;
{
...
void flushCache(messageToSend, class) object messageToSend, class;
{
  int hash;
...
*/
static boolean findMethod(methodClassLocation) object *methodClassLocation;
{
  ...
  for (; methodClass != nilobj;
       methodClass = basicAt(methodClass, superClassInClass))
  {
...
boolean execute(aProcess, maxsteps) object aProcess;
int maxsteps;
{
          ...
          case contextConst:
            ...
            if (contextObject == processStack)
            {
              ...
              contextObject = newContext(
                linkPointer, method,
                copyFrom(processStack, returnPoint, linkPointer - returnPoint),
