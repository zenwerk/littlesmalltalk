...
void block()
{
  ...
  if ((token == binary) && streq(tokenString, ":"))
  {
    ...
    if ((token != binary) || !streq(tokenString, "|"))
      compilError(selector, "block argument list must be terminated", "by |");
    ignore nextToken();
  }
  newBlk = newBlock();
...
void messagePattern()
{
  ...
  { /* binary message pattern */
    ignore nextToken();
    if (token != nameconst)
      compilError(selector, "binary message pattern not followed by name",
                  selector);
...
boolean parse(method, text, savetext) object method;
char *text;
boolean savetext;
{
  int i;
