...
struct object *nilObject, *trueObject, *falseObject, *SmallIntClass,
  *ArrayClass, *BlockClass, *ContextClass, *globalsObject, *initialMethod,
 ...
 */
static struct object *newLInteger(long long val)
{
  struct object *res;
                       ...
                       struct object *repStart)
{
  ...
  if ((src->size & FLAG_BIN) && (dest->size & FLAG_BIN))
  {
     ...
     * Do it.
     */
    bcopy(bytePtr(src) + irepStart, bytePtr(dest) + istart, count);
    return (0);
  }
   ...
   */
  bcopy(&src->data[irepStart], &dest->data[istart], BytesPerWord * count);
  return (0);
}
...
int execute(struct object *aProcess, int ticks)
{
  ...
  struct object *context, *method, *arguments, *temporaries, *instanceVariables,
    *literals, *stack, *returnedValue = nilObject, *messageSelector,
                       *receiverClass, *op;
  unsigned char *bp;
  long long l;
  ...
  for (;;)
  {
    ...
    switch (high)
    {
      ...
      findMethodFromSymbol:
        ...
        DBGS("SendMessage", bytePtr(receiverClass->data[nameInClass]),
             bytePtr(messageSelector));
      checkCache:
        low =
          (((UINT_PTR)messageSelector) + ((UINT_PTR)receiverClass)) % cacheSize;
        ...
        switch (high)
        {
          ...
          case 10: /* small integer addition */
                ...
                ((high < 0) && (low < 0) && (x > high)))
            {
              ...
              returnedValue = newLInteger((long long)high + (long long)low);
            }
            else
            {
              if (!FITS_SMALLINT(x))
          ...
          case 22: /* string at put */
            ...
            bytePtr(returnedValue)[low] = integerValue(stack->data[--stackTop]);
            break;
...
#undef GET_HIGH_LOW
          ...
          case 38: /* replaceFrom:... */
            returnedValue = stack->data[--stackTop];
            stackTop -= 4;
            if (bulkReplace(returnedValue, stack->data[stackTop + 4],
