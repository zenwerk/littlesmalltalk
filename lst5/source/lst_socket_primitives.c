                                  ...
                                  int *failed)
{
  ...
  switch (integerValue(args->data[0]))
  {
    ...
    case 0: /* constants */
      ...
      {
        case 0:
          returnedValue = newInteger(AF_INET);
          break;
        case 1:
          returnedValue = newInteger(AF_INET6);
          break;
        case 2:
          returnedValue = newInteger(AF_LOCAL);
          break;
        case 10:
          returnedValue = newInteger(SOCK_STREAM);
          break;
        case 11:
          returnedValue = newInteger(SOCK_DGRAM);
          break;
        case 12:
          returnedValue = newInteger(SOCK_RAW);
          break;
        case 20:
          returnedValue = newInteger(IPPROTO_TCP);
          break;
        case 21:
          returnedValue = newInteger(IPPROTO_UDP);
          break;
#ifdef IPPROTO_SCTP
        case 22:
          returnedValue = newInteger(IPPROTO_SCTP);
          break;
#endif
        case 30:
          returnedValue = newInteger(SHUT_RD);
          break;
        case 31:
          returnedValue = newInteger(SHUT_WR);
          break;
        case 32:
          returnedValue = newInteger(SHUT_RDWR);
          break;
      }
      break;
...
#endif
      int rc =
        connect(sockfd, (struct sockaddr *)addrObj->bytes, SIZE(addrObj));
#if defined(DEBUG) && defined(TRACE)
    ...
    { /* recvfrom */
      ...
      ssize_t got =
        recvfrom(sockfd, bufObj->bytes, n, 0, (struct sockaddr *)&ss, &len);
...
#define LST_WAIT_W 2
      ...
      if (waitfor & LST_WAIT_R)
      {
        FD_ZERO(&rfds);
        FD_SET(fd, &rfds);
        rp = &rfds;
      }
      if (waitfor & LST_WAIT_W)
      {
        FD_ZERO(&wfds);
        FD_SET(fd, &wfds);
        wp = &wfds;
      ...
      {
        rc = 0;
        if ((waitfor & LST_WAIT_R) && FD_ISSET(fd, &rfds))
          rc |= LST_WAIT_R;
        if ((waitfor & LST_WAIT_W) && FD_ISSET(fd, &wfds))
          rc |= LST_WAIT_W;
      }
#undef LST_WAIT_R
#undef LST_WAIT_W
                                ...
                                int *failed)
{
  ...
  switch (integerValue(args->data[0]))
  {
    ...
    case 0:
    { /* Inet firstIpStringFromHostname: hostName -> IPv4 address string */
      char hostname[256];
    ...
    case 1:
    { /* Inet firstIpAddressFromHostname: hostName -> IPv4 in_addr binary */
      char hostname[256];
      lstGetString(hostname, sizeof(hostname), args->data[1]);
      struct hostent *h = gethostbyname(hostname);
      if (h && h->h_addrtype == AF_INET)
    ...
    case 2:
    { /* Inet newIpAddressFromIpString: ipStr -> IPv4 in_addr binary */
      char ipStr[16];
    ...
    case 3:
    { /* Inet newSocketAddressFromIpAddress: in_addr port: port -> socketaddr_in
         binary */
    ...
    case 4:
    { /* Inet newSocketAddressFromIpString: ipStr port: port -> socketaddr_in
         binary */
      char ipStr[16];
      lstGetString(ipStr, sizeof(ipStr), args->data[1]);
      struct in_addr addr;
    ...
    case 5:
    { /* Inet newSocketAddressFromHostname: host port: port -> socketaddr_in
         binary */
      char hostname[256];
      lstGetString(hostname, sizeof(hostname), args->data[1]);
      struct hostent *h = gethostbyname(hostname);
