...
extern "C"
{
   ...
   *
   * \verbatim im.FileOpenRaw(file_name: string) -> ifile: imFile, error: number
   * [in Lua 5] \endverbatim \ingroup raw */
  imFile* imFileOpenRaw(const char* file_name, int* error);

  /** Creates a RAW image file.
   * See also \ref imErrorCodes.
   *
   * \verbatim im.FileNewRaw(file_name: string) -> ifile: imFile, error: number
   * [in Lua 5] \endverbatim \ingroup raw */
