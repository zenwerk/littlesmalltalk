 ...
 *
 * \par
 * Usage is just like the C API. Open and New are replaced by equivalent
 * constructors. \n Close is replaced by the destructor. Error checking is done
 * by the Error() member. \n Open and New errors are cheked using the Failed()
 * member. \ingroup file */
class imImageFile
{
  imFile* ifile;
...
public:
  ...
  {
    if (this->ifile)
      imFileClose(this->ifile);
  }

  int Failed() { return this->ifile == 0; }
  ...
  void ReadImageInfo(int index, int* width, int* height, int* color_mode,
                     int* data_type)
  {
    this->error = imFileReadImageInfo(this->ifile, index, width, height,
                                      color_mode, data_type);
  }
  ...
  {
    this->error =
      imFileReadImageData(this->ifile, data, convert2bitmap, color_mode_flags);
  }
  ...
  {
    this->error =
      imFileWriteImageInfo(this->ifile, width, height, color_mode, data_type);
  }
