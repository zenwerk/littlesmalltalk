 ...
 * We use the hash function described in "The Pratice of Programming" of
 * Kernighan & Pike. \ingroup util */
class imAttribTable
{
...
public:
   ...
   * If size is zero the default size of 101 is used. Size must be a prime
   * number. Other common values are 67, 599 and 1499.*/
  imAttribTable(int hash_size) { ptable = imAttribTableCreate(hash_size); }
  ...
  int Count() const { return imAttribTableCount(ptable); }

  /** Removes all the attributes in the table */
  void RemoveAll() { imAttribTableRemoveAll(ptable); }
  ...
  void UnSet(const char* name) { imAttribTableUnSet(ptable, name); }
  ...
  /** For each attribute calls the user callback. If the callback returns 0 the
   * function returns. */
...
public:
  ...
  imAttribArray(int count) { ptable = imAttribArrayCreate(count); }
   ...
   * Returns the attribute if found, NULL otherwise.
   * See also \ref imDataType. */
  const void* Get(int index, char* name = 0, int* data_type = 0,
                  int* count = 0) const
  {
