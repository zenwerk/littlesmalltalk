...
#define IM_DECL
   ...
   *
   * \verbatim im.VideoCaptureDeviceCount() -> count: number [in Lua 5]
   * \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureDeviceCount(void);

  /** Returns the device description. Returns NULL only if it is an invalid
   * device.
   *
   * \verbatim im.VideoCaptureDeviceDesc(device: number) -> desc: string [in Lua
   * 5] \endverbatim \ingroup capture */
  const char* IM_DECL imVideoCaptureDeviceDesc(int device);

  /** Returns the extendend device description. May return NULL.
   *
   * \verbatim im.VideoCaptureDeviceExDesc(device: number) -> desc: string [in
   * Lua 5] \endverbatim \ingroup capture */
  const char* imVideoCaptureDeviceExDesc(int device);

  /** Returns the device path configuration. This is a unique string.
   *
   * \verbatim im.VideoCaptureDevicePath(device: number) -> desc: string [in Lua
   * 5] \endverbatim \ingroup capture */
  const char* imVideoCaptureDevicePath(int device);

  /** Returns the vendor information. May return NULL.
   *
   * \verbatim im.VideoCaptureDeviceVendorInfo(device: number) -> desc: string
   * [in Lua 5] \endverbatim \ingroup capture */
   ...
   *
   * \verbatim im.imVideoCaptureReloadDevices() -> count: number [in Lua 5]
   * \endverbatim \ingroup capture */
   ...
   *
   * \verbatim im.VideoCaptureCreate() -> vc: imVideoCapture [in Lua 5]
   * \endverbatim \ingroup capture */
  imVideoCapture* IM_DECL imVideoCaptureCreate(void);

  /** Destroys a imVideoCapture object.
   *
   * \verbatim im.VideoCaptureDestroy(vc: imVideoCapture) [in Lua 5]
   * \endverbatim \verbatim vc:Destroy() [in Lua 5] \endverbatim \ingroup
   * capture */
   ...
   *
   * \verbatim vc:Connect([device: number]) -> ret: number [in Lua 5]
   * \endverbatim \ingroup capture */
   ...
   * Returns zero if failed.
   *
   * \verbatim vc:ShowDialog(dialog: number, parent: userdata) -> error: number
   * [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureShowDialog(imVideoCapture* vc, int dialog,
                                       void* parent);

  /** Allows to control the input and output of devices that have multiple input
   * and outputs. cross controls in which stage the input/output will be set.
   * Usually use 1, but some capture boards has a second stage. In Direct X it
   * controls the crossbars.
   *
   * \verbatim vc:SetInOut(input, output, cross: number) -> error: number [in
   * Lua 5] \endverbatim \ingroup capture */
  int imVideoCaptureSetInOut(imVideoCapture* vc, int input, int output,
                             int cross);

  /** Returns the description of a configuration dialog.
   * dialog can be from 0 to \ref imVideoCaptureDialogCount. \n
   *
   * \verbatim vc:DialogDesc(dialog: number) -> desc: string [in Lua 5]
   * \endverbatim \ingroup capture */
   ...
   * Returns zero if failed.
   *
   * \verbatim vc:GetFormat(format: number) -> error: number, width: number,
   * height: number, desc: string [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureGetFormat(imVideoCapture* vc, int format,
                                      int* width, int* height, char* desc);
   ...
   * \n When the format is changed in the dialog, for some formats the returned
   * format is the preferred format, not the current format. \n This will not
   * affect color_mode of the capture image. \n Returns zero if failed.
   *
   * \verbatim vc:SetFormat(format: number) -> error: number [in Lua 5]
   * \endverbatim \ingroup capture */
   ...
   *
   * \verbatim vc:GetImageSize() -> width: number, height: number [in Lua 5]
   * \endverbatim \ingroup capture */
  void IM_DECL imVideoCaptureGetImageSize(imVideoCapture* vc, int* width,
                                          int* height);
   ...
   * Returns zero if failed.
   *
   * \verbatim vc:SetImageSize(width: number, height: number) -> error: number
   * [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureSetImageSize(imVideoCapture* vc, int width,
                                         int height);

  /** Returns a new captured frame. Use -1 for infinite timeout. \n
   * Color space can be IM_RGB or IM_GRAY, and mode can be packed (IM_PACKED) or
   * not. \n It can not have an alpha channel and orientation is always bottom
   * up. \n Returns zero if failed or timeout expired, the buffer is not
   * changed.
   *
   * \verbatim vc:Frame(image: imImage, timeout: number) -> error: number [in
   * Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureFrame(imVideoCapture* vc, unsigned char* data,
                                  int color_mode, int timeout);
   ...
   * Returns zero if failed.
   *
   * \verbatim vc:OneFrame(image: imImage) -> error: number [in Lua 5]
   * \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureOneFrame(imVideoCapture* vc, unsigned char* data,
                                     int color_mode);
   ...
   * Returns zero if failed.
   *
   * \verbatim vc:ResetAttribute(attrib: string, fauto: number) -> error: number
   * [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureResetAttribute(imVideoCapture* vc,
                                           const char* attrib, int fauto);

  /** Returns a camera or video attribute in percentage of the valid range
   * value. \n Returns zero if failed or attribute not supported.
   *
   * \verbatim vc:GetAttribute(attrib: string) -> error: number, percent: number
   * [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureGetAttribute(imVideoCapture* vc, const char* attrib,
                                         float* percent);

  /** Changes a camera or video attribute in percentage of the valid range
   * value. \n Returns zero if failed or attribute not supported.
   *
   * \verbatim vc:SetAttribute(attrib: string, percent: number) -> error: number
   * [in Lua 5] \endverbatim \ingroup capture */
  int IM_DECL imVideoCaptureSetAttribute(imVideoCapture* vc, const char* attrib,
                                         float percent);

  /** Returns a list of the description of the valid attributes for the device
   * class. \n But each device may still not support some of the returned
   * attributes. \n Use the return value of \ref imVideoCaptureGetAttribute to
   * check if the attribute is supported.
   *
   * \verbatim vc:GetAttributeList() -> attrib_list: table of strings [in Lua 5]
   * \endverbatim \ingroup capture */
   ...
   * Use the return value of \ref imVideoCaptureGetAttribute to check if the
  attribute is supported. \verbatim VideoBrightness - Specifies the brightness,
  also called the black level. VideoContrast - Specifies the contrast, expressed
  as gain factor. VideoHue - Specifies the hue angle. VideoSaturation -
  Specifies the saturation. VideoSharpness - Specifies the sharpness. VideoGamma
  - Specifies the gamma. VideoColorEnable - Specifies the color enable setting.
  (0/100) VideoWhiteBalance - Specifies the white balance, as a color
  temperature in degrees Kelvin. VideoBacklightCompensation - Specifies the
  backlight compensation setting. (0/100) VideoGain - Specifies the gain
  adjustment. CameraPanAngle - Specifies the camera's pan angle. To 100 rotate
  right, To 0 rotate left (view from above). CameraTiltAngle - Specifies the
  camera's tilt angle.  To 100 rotate up, To 0 rotate down. CameraRollAngle -
  Specifies the camera's roll angle. To 100 rotate right, To 0 rotate left.
    ...
    CameraFocus - Specifies the camera's focus setting, as the distance to the
  optimally focused target. FlipHorizontal - Specifies the video will be flipped
  in the horizontal direction. FlipVertical - Specifies the video will be
  flipped in the vertical direction. AnalogFormat - Specifies the video format
  standard NTSC, PAL, etc. Valid values: NTSC_M     = 0 NTSC_M_J   = 1 NTSC_433
  = 2 PAL_B      = 3 PAL_D      = 4 PAL_H      = 5 PAL_I      = 6 PAL_M      = 7
        PAL_N      = 8
        PAL_60     = 9
        SECAM_B    = 10
        SECAM_D    = 11
...
{
public:
  imCapture() { vc = imVideoCaptureCreate(); }
  ...
  int Connect(int device) { return imVideoCaptureConnect(vc, device); }

  void Disconnect() { imVideoCaptureDisconnect(vc); }

  int DialogCount() { return imVideoCaptureDialogCount(vc); }
  ...
  int FormatCount() { return imVideoCaptureFormatCount(vc); }
  ...
  int SetFormat(int format) { return imVideoCaptureSetFormat(vc, format); }
  ...
  int Live(int live) { return imVideoCaptureLive(vc, live); }
