 ...
 *
 * \par
 * Virtual Base class for file formats. All file formats inherit from this
 * class. \ingroup filesdk */
...
public:
  ...
  /* Pure Virtual Methods. Every driver must implement all the following
   * methods. */

  virtual int Open(
    const char* file_name) = 0; // Must initialize compression and image_count
  ...
  virtual int CanWrite(const char* compression, int color_mode,
                       int data_type) const = 0;
};
...
extern "C"
{
   ...
   * Used by "im_file.cpp" only. */
  imFormat* imFormatOpenAs(const char* file_name, const char* format,
                           int* error);
