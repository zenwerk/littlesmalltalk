...
extern "C"
{
   ...
   *
   * \par
   * Windows DIBs in memory are handled just like a BMP file without the file
   * header. \n These functions will work only in Windows. They are usefull for
   * interchanging data with the clipboard, with capture drivers, with the AVI
   * and WMF file formats and others. \par Supported DIB aspects: \li bpp must
   * be 1, 4, 8, 16, 24, or 32. \li BITMAPV4HEADER or BITMAPV5HEADER are handled
   * but ignored. \n \li BITMAPCOREHEADER is not handled . \li BI_JPEG and
   * BI_PNG compressions are not handled. \li biHeight can be negative,
   * compression can be RLE only if created from imDibCreateReference,
   * imDibPasteClipboard, imDibLoadFile. \li can not encode/decode Images
   * to/from RLE compressed Dibs. \li if working with RLE Dibs bits_size is
   * greatter than used. \li the resolution of a new Dib is taken from the
   * screen. \li SetDIBitsToDevice(start_scan is 0, scan_lines is
   * dib->bmih->biHeight). \li StretchDIBits(use always DIB_RGB_COLORS). \li
   * CreateDIBPatternBrushPt(packed_dib is dib->dib). \par Must include
   * <windows.h> before using these functions. \n Check <wingdi.h> for
   * structures and definitions. \par See \ref im_dib.h \ingroup util */
  ...
  typedef struct _imDib
  {
    ...
    int pad_size; /**< number of bytes remaining in the line, lines are in a
                     word boundary */
   ...
   * \ingroup dib */
  imDib* imDibCreateSection(HDC hDC, HBITMAP* image, int width, int height,
                            int bpp);
   ...
   * \ingroup dib */
  typedef void (*imDibLineSetPixel)(unsigned char* line, int col,
                                    unsigned int pixel);
   ...
   * imDibCopyClipboard(imDibCreateCopy(dib)). Warning: Clipboard functions in
   * C++ can fail with Visual C++ /EHsc (Enable C++ Exceptions) \ingroup dib */
   ...
   * /EHsc (Enable C++ Exceptions) \ingroup dib */
   ...
   * \ingroup dib */
  void imDibDecodeToRGBA(const imDib* dib, unsigned char* red,
                         unsigned char* green, unsigned char* blue,
                         unsigned char* alpha);

  /** Converts a DIB into an indexed image. bpp must be <=8. colors must have
   * room for at least 256 colors. colors is rgb packed (RGBRGBRGB...)
   * \ingroup dib */
   ...
   * alpha is used only when bpp=32.
   * \ingroup dib */
  void imDibEncodeFromRGBA(imDib* dib, const unsigned char* red,
   ...
   * \ingroup dib */
  void imDibEncodeFromMap(imDib* dib, const unsigned char* map,
                          const long* palette, int palette_count);
...
#ifdef __IM_IMAGE_H
  /* You must include "im_image.h" before this header to enable these
   * declarations. */
