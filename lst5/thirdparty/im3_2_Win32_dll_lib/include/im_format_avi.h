...
extern "C"
{
   ...
   * \par
   * Access to the AVI format uses Windows AVIFile library. Available in Windows
  Only. \n
   * When writing a new file you must use an ".avi" extension, or the Windows
  API will fail. \n
  ...
  always as bottom up. Handle(0) returns NULL. imBinFile is not supported.
      ...
      Attributes:
        ...
        KeyFrameRate IM_INT (1) (write only) [key frame frequency, if 0 not
  using key frames, default 15] DataRate IM_INT (1) (write only)
  [kilobits/second, default 2400]

      Comments:
        Reads only the first video stream. Other streams are ignored.
        All the images have the same size, you must call
  imFileReadImageInfo/imFileWriteImageInfo at least once. For codecs comparsion
  and download go to: http://graphics.lcs.mit.edu/~tbuehler/video/codecs/
          http://www.fourcc.org
  \endverbatim
   * \ingroup format */
