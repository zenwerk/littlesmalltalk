...
extern "C"
{
   ...
   * the current CPU byte order. It can invert 2, 4 or 8 bytes numbers to/from
   * little/big-endian orders. \par It will process the data only if the file
   * format is diferent from the current CPU. \par Can read from disk or memory.
   * In case of a memory buffer, the file name must be the \ref
   * imBinMemoryFileName structure. \par See \ref im_binfile.h \ingroup util */
  ...
  /** Reads an array of count values with byte sizes: 1, 2, 4, or 8. And invert
   * the byte order if necessary after read. \ingroup binfile */
  unsigned long imBinFileRead(imBinFile* bfile, void* pValues,
                              unsigned long pCount, int pSizeOf);

  /** Writes an array of values with sizes: 1, 2, 4, or 8. And invert the byte
   * order if necessary before write.\n <b>ATENTION</b>: The function will not
   * make a temporary copy of the values to invert the byte order.\n So after
   * the call the values will be invalid, if the file byte order is diferent
   * from the CPU byte order. \ingroup binfile */
  unsigned long imBinFileWrite(imBinFile* bfile, void* pValues,
                               unsigned long pCount, int pSizeOf);
  ...
  enum imBinFileModule
  {
    ...
    IM_SUBFILE, /**< It is a sub file. FileName is a imBinFile* pointer from any
                   other module. */
    IM_FILEHANDLE, /**< System dependent file I/O Rotines, but FileName is a
                      system file handle ("int" in UNIX and "HANDLE" in
                      Windows). */
  ...
  {
    unsigned char*
      buffer; /**< The memory buffer. If you are reading the buffer must exists.
               *   If you are writing the buffer can be internally allocated to
               * the given size. The buffer is never free. The buffer is
               * allocated using "malloc", and reallocated using "realloc". Use
               * "free" to release it. To avoid RTL conflicts use the function
               * imBinMemoryRelease. */
    int size; /**< Size of the buffer. */
    float
      reallocate; /**< Reallocate factor for the memory buffer when writing
                   * (size += reallocate*size). Set reallocate to 0 to disable
                   * reallozation, in this case buffer must not be NULL. */
  } imBinMemoryFileName;
...
public:
  ...
  unsigned long Read(void* pValues, unsigned long pCount, int pSizeOf)
  {
    ...
    if (pSizeOf != 1 && DoByteOrder)
      imBinSwapBytes(pValues, pCount, pSizeOf);
