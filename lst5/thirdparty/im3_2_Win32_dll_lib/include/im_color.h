 ...
 * \li www.littlecms.com - A free Color Management System (use this if you need
precise color conversions)
 *
 * \section cci Color Component Intervals
 * \par
 * All the color components are stored in the 0-max interval, even the signed
ones. \n
 * Here are the pre-defined intervals for each data type. These values are used
for standard color conversion.
 ...
 * This is a value to be compensated when the data_type is unsigned and
 * component is signed. \n \ingroup color */
...
{
  if (max == 1)
    return (T)value; // to allow a dummy quantize
  if (value >= 1)
    return max;
  if (value <= 0)
    return 0;
 ...
 * r = (q + 0.5)/(max + 1)  \n
 * Add 0.5 to set the same origin, then multiply by the size of each interval
 * 1/(max+1). \n But 0 is mapped to 0, and max is mapped to 1. \ingroup color */
template <class T>
inline float imColorReconstruct(const T& value, const T& max)
{
  if (max == 1)
    return (float)value; // to allow a dummy reconstruct
  if (value <= 0)
    return 0;
  if (value >= max)
    return 1;
 ...
 * \ingroup color */
template <class T>
inline void imColorYCbCr2RGB(const T Y, const T Cb, const T Cr, T& R, T& G,
                             T& B, const T& zero, const T& max)
{
 ...
 * \ingroup color */
template <class T>
inline void imColorRGB2YCbCr(const T R, const T G, const T B, T& Y, T& Cb,
                             T& Cr, const T& zero)
{
 ...
 * \ingroup color */
template <class T>
inline void imColorCMYK2RGB(const T C, const T M, const T Y, const T K, T& R,
                            T& G, T& B, const T& max)
{
  T W = max - K;
  R = (T)((W * (max - C)) / max);
 ...
 * \ingroup color */
template <class T>
inline void imColorXYZ2RGB(const T X, const T Y, const T Z, T& R, T& G, T& B,
                           const T& max)
{
 ...
 * \ingroup color */
template <class T>
inline void imColorRGB2XYZ(const T R, const T G, const T B, T& X, T& Y, T& Z)
{
...
#define IM_FWLAB(_w) \
  (_w > 0.008856f ? powf(_w, 1.0f / 3.0f) : 7.787f * _w + 0.16f / 1.16f)
...
#define IM_GWLAB(_w) \
  (_w > 0.20689f ? powf(_w, 3.0f) : 0.1284f * (_w - 0.16f / 1.16f))
 ...
 * We use the sRGB transfer function. sRGB uses ITU-R 709 primaries and D65
 ...
 * \ingroup color */
inline void imColorRGB2RGBNonlinear(const float RL, const float GL,
                                    const float BL, float& R, float& G,
                                    float& B)
{
