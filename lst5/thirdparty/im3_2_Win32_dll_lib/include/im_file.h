...
extern "C"
{
   ...
   * \par
   * All the file formats are based on theses structures. Use them to create new
   * file formats. \n The LineBuffer functions will help transfer image from
   * format buffer to application buffer and vice-versa. \par See \ref im_file.h
   * \ingroup file */
  ...
  struct _imFile
  {
    int is_new;
    void*
      attrib_table; /**< in fact is a imAttribTable, but we hide this here */

    void* line_buffer; /**< used for line convertion, contains all components if
                          packed, or only one if not */
    ...
    int convert_bpp; /**< number of bpp to unpack/pack to/from 1 byte.
                          When reading converts n packed bits to 1 byte
                        (unpack). If n>1 will also expand to 0-255. When writing
                        converts 1 byte to 1 bit (pack). If negative will only
                        expand to 0-255 (no unpack or pack). */
    int switch_type; /**< flag to switch the original data type: char-byte,
                        short-ushort, uint-int, double-float */

    long palette[256];
    int palette_count;

    int user_color_mode, user_data_type,
    ...
    char compression[10];
    int image_count, image_index, width, height;
  };
   ...
   * \ingroup filesdk */
  void imFileLineBufferWrite(imFile* ifile, const void* data, int line,
                             int plane);
