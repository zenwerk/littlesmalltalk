...
extern "C"
{
   ...
   * http://www.lua.org
   * \par
   * The name of the functions were changed because of the namespace "im" and
   ...
   *
   * \verbatim image:cdPutImageRect(x: number, y: number, w: number, h: number,
   xmin: number, xmax: number, ymin: number, ymax: number) [in Lua 5] Draws the
   imImage into the active cdCanvas. The imImage must be a bitmap image, see
   imImageIsBitmap. \endverbatim
   *
   * \verbatim image:wdPutImageRect(x: number, y: number, w: number, h: number,
   xmin: number, xmax: number, ymin: number, ymax: number) [in Lua 5] Draws the
   imImage into the active cdCanvas using world coordinates. The imImage must be
   a bitmap image, see imImageIsBitmap. \endverbatim
   *
   ...
   *
   * \verbatim image:cdCreateCanvas([res: number]) -> canvas: cdCanvas [in Lua
   5] Creates a cdCanvas using the CD_IMAGERGB driver. Resolution is optional,
   default is 3.8 pixels per milimiter (96.52 DPI). The imImage must be a
   IM_RGB/IM_BYTE image. \endverbatim
   *
   * \ingroup imlua */
