...
extern "C"
{
   ...
   * \par
   * Creates several standard palettes. The palette is just an array of encoded
   * color values. See also \ref colorutl. \par In Lua, to create a palette you
   * can call im.PaletteCreate. \verbatim im.PaletteCreate([count: number]) ->
   * pal: imPalette [in Lua 5] \endverbatim Default count is 256. IMLua and
   * CDLua palettes are 100% compatible. \par See \ref im_palette.h \ingroup
   * util */

  /** Searches for the nearest color on the table and returns the color index if
   * successful. It looks in all palette entries and finds the minimum euclidian
   * square distance. If the color matches the given color it returns
   * immediately. See also \ref colorutl.
   *
   * \verbatim im.PaletteFindNearest(pal: imPalette, color: lightuserdata) ->
   * index: number [in Lua 5] \endverbatim \ingroup palette */
   ...
   * successful. If the tolerance is 0 search for the exact match in the palette
   * else search for the first color that fits in the tolerance range. See also
   * \ref colorutl.
   *
   * \verbatim im.PaletteFindColor(pal: imPalette, color: lightuserdata, tol:
   * number) -> index: number [in Lua 5] \endverbatim \ingroup palette */
  int imPaletteFindColor(const long* palette, int palette_count, long color,
                         unsigned char tol);
   ...
   * The colors are arranged in the light wave length spectrum order (starting
   * from purple).
   *
   ...
   *
   * \verbatim im.PaletteHighContrast() -> pal: imPalette [in Lua 5]
   * \endverbatim \ingroup palette */
   ...
   *
   * \verbatim im.PaletteUniformIndex(color: lightuserdata) -> index: number [in
   * Lua 5] \endverbatim \ingroup palette */
   ...
   *
   * \verbatim im.PaletteUniformIndexHalftoned(color: lightuserdata, x: number,
   * y: number) -> index: number [in Lua 5] \endverbatim \ingroup palette */
