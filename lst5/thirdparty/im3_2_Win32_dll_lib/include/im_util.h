...
#define IM_MAX(_a, _b) (_a > _b ? _a : _b)
   ...
   *
   * \verbatim im.ImageDataSize(width: number, height: number, color_mode:
   * number, data_type: number) -> datasize: number [in Lua 5] \endverbatim
   * \ingroup imageutil */
   ...
   *
   * \verbatim im.ImageLineSize(width: number, color_mode: number, data_type:
   * number) -> linesize: number [in Lua 5] \endverbatim \ingroup imageutil */
   ...
   * components, if not includes only one.
   *
   * \verbatim im.ImageLineCount(width: number, color_mode: number) ->
   * linecount: number [in Lua 5] \endverbatim \ingroup imageutil */
  int imImageLineCount(int width, int color_mode);

  /** Check if the combination color_mode+data_type is valid.
   *
   * \verbatim im.ImageCheckFormat(color_mode: number, data_type: number) ->
   * check: boolean [in Lua 5] \endverbatim \ingroup imageutil */
   ...
   *
   * \verbatim im.ColorEncode(red: number, green: number, blue: number) ->
   * color: lightuserdata [in Lua 5] \endverbatim \ingroup colorutl */
   ...
   * "long" definition is compatible with the CD library definition.
   *
   * \verbatim im.ColorDecode(color: lightuserdata) -> red: number, green:
   * number, blue: number [in Lua 5] \endverbatim \ingroup colorutl */
   ...
   *
   * \verbatim im.ColorModeSpaceName(color_mode: number) -> name: string [in Lua
   * 5] \endverbatim \ingroup colormodeutl */
  const char* imColorModeSpaceName(int color_mode);

  /** Returns the number of components of the color space including alpha.
   *
   * \verbatim im.ColorModeDepth(color_mode: number) -> depth: number [in Lua 5]
   * \endverbatim \ingroup colormodeutl */
  int imColorModeDepth(int color_mode);

/** Returns the color space of the color mode.
 *
 * \verbatim im.ColorModeSpace(color_mode: number) -> color_space: number [in
 * Lua 5] \endverbatim \ingroup colormodeutl */
#define imColorModeSpace(_cm) (_cm & 0xFF)

/** Check if the two color modes match. Only the color space is compared.
 *
 * \verbatim im.ColorModeMatch(color_mode1: number, color_mode2: number) ->
 * match: boolean [in Lua 5] \endverbatim \ingroup colormodeutl */
#define imColorModeMatch(_cm1, _cm2) \
  (imColorModeSpace(_cm1) == imColorModeSpace(_cm2))

/** Check if the color mode has an alpha channel.
 *
 * \verbatim im.ColorModeHasAlpha(color_mode: number) -> has_alpha: boolean [in
 * Lua 5] \endverbatim \ingroup colormodeutl */
#define imColorModeHasAlpha(_cm) (_cm & IM_ALPHA)

/** Check if the color mode components are packed in one plane.
 *
 * \verbatim im.ColorModeIsPacked(color_mode: number) -> is_packed: boolean [in
 * Lua 5] \endverbatim \ingroup colormodeutl */
#define imColorModeIsPacked(_cm) (_cm & IM_PACKED)

/** Check if the color mode orients the image from top down to bottom.
 *
 * \verbatim im.ColorModeIsTopDown(color_mode: number) -> is_top_down: boolean
 * [in Lua 5] \endverbatim \ingroup colormodeutl */
#define imColorModeIsTopDown(_cm) (_cm & IM_TOPDOWN)

  /** Returns the color space of the equivalent display bitmap image. \n
   * Original packing and alpha are ignored. Returns IM_RGB, IM_GRAY, IM_MAP or
   * IM_BINARY.
   *
   * \verbatim im.ColorModeToBitmap(color_mode: number) -> color_space: number
   * [in Lua 5] \endverbatim \ingroup colormodeutl */
  int imColorModeToBitmap(int color_mode);

  /** Check if the color mode and data_type defines a display bitmap image.
   *
   * \verbatim im.ColorModeIsBitmap(color_mode: number, data_type: number) ->
   * is_bitmap: boolean [in Lua 5] \endverbatim \ingroup colormodeutl */
...
#define IM_CROPMAX(_v, _max) (_v < 0 ? 0 : _v > _max ? _max : _v)
   ...
   *
   * \verbatim im.DataTypeSize(data_type: number) -> size: number [in Lua 5]
   * \endverbatim \ingroup datatypeutl */
  int imDataTypeSize(int data_type);

  /** Returns the numeric data type name given its identifier.
   *
   * \verbatim im.DataTypeName(data_type: number) -> name: string [in Lua 5]
   * \endverbatim \ingroup datatypeutl */
   ...
   * returns 0.
   *
   * \verbatim im.DataTypeIntMax(data_type: number) -> int_max: number [in Lua
   * 5] \endverbatim \ingroup datatypeutl */
  unsigned long imDataTypeIntMax(int data_type);

  /** Returns the minimum value of an integer data type. For floating point
   * returns 0.
   *
   * \verbatim im.DataTypeIntMin(data_type: number) -> int_min: number [in Lua
   * 5] \endverbatim \ingroup datatypeutl */
  ...
  enum imByteOrder
  {
    IM_LITTLEENDIAN, /**< Little Endian - The most significant byte is on the
                        right end of a word. Used by Intel processors. */
    IM_BIGENDIAN /**< Big Endian - The most significant byte is on the left end
                    of a word. Used by Motorola processors, also is the network
                    standard byte order. */
  };
   ...
   * The destination buffer must be at least 0.1% larger than source_size plus
   * 12 bytes. \n It compresses raw byte data. zip_quality can be 1 to 9. \n
   * Returns the size of the compressed buffer or zero if failed.
   * \ingroup compress */
  int imCompressDataZ(const void* src_data, int src_size, void* dst_data,
                      int dst_size, int zip_quality);

  /** Uncompresses the data compressed with the ZLIB Deflate compression. \n
   * Returns zero if failed.
   * \ingroup compress */
  int imCompressDataUnZ(const void* src_data, int src_size, void* dst_data,
                        int dst_size);

  /** Compresses the data using the libLZF compression. \n
   * Returns the size of the compressed buffer or zero if failed.
   * \ingroup compress */
  int imCompressDataLZF(const void* src_data, int src_size, void* dst_data,
                        int dst_size, int zip_quality);

  /** Uncompresses the data compressed with the libLZF compression.
   * Returns zero if failed.
   * \ingroup compress */
  int imCompressDataUnLZF(const void* src_data, int src_size, void* dst_data,
                          int dst_size);
