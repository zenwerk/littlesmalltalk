...
extern "C"
{
   ...
   * the Aldus Corporation keeped the copyright until Aldus was aquired by
  Adobe. \n
  ...
  \verbatim
      ...
      Compressions:
        ...
        SGILOG - SGI Log Luminance RLE for L and Luv (only byte, ushort and
  float) [default for XYZ] SGILOG24 - SGI Log 24-bit packed for Luv (only byte,
  ushort and float) Can have more than one image. Can have an alpha channel.
      ...
      Attributes:
        Photometric IM_USHORT (1) (when writing this will complement the
  color_mode information, for Mask, MinIsWhite, ITULab and ICCLab)
        ExtraSampleInfo IM_USHORT (1) (description of alpha channel: 0- uknown,
  1- pre-multiplied, 2-normal) JPEGQuality IM_INT (1) [0-100, default 75] (write
  only) ZIPQuality IM_INT (1) [1-9, default 6] (write only) ResolutionUnit
  (string) ["DPC", "DPI"] XResolution, YResolution IM_FLOAT (1) Description,
  Author, Copyright, DateTime, DocumentName, PageName, TargetPrinter, Make,
  Model, Software, HostComputer (string) InkNames (strings separated by '0's)
        InkSet IM_USHORT (1)
        NumberOfInks IM_USHORT (1)
        DotRange IM_USHORT (2)
        TransferFunction0, TransferFunction1, TransferFunction3 IM_USHORT
  [gray=0, rgb=012] ReferenceBlackWhite IMFLOAT (6) WhitePoint IMFLOAT (2)
  ...
  GeoDoubleParams IM_FLOAT (N) GeoASCIIParams (string) (other attributes can be
  obtained by using libTIFF directly using the Handle(1) function)

      Comments:
        LogLuv is in fact Y'+CIE(u,v), so we choose to convert to XYZ.
        SubIFD is not handled.
        Since LZW patent expired, LZW compression is enabled. LZW Copyright
  Unisys. libGeoTIFF can be used without XTIFF initialization. Use Handle(1) to
  obtain a TIFF*.
  ...
  \verbatim
      ...
      Changes to libJPEG:
        ...
        jinclude.h - standard JFFLUSH and JFERROR definitions, and new macro
  HAVE_JFIO. jmorecfg.h - changed definition of INT32 to JINT32 for better
  compatibility. jdhuf.c - added support for OJPEG_SUPPORT in libTIFF. new file
  created: jconfig.h
      ...
      Comments:
        ...
        Also YcbCr are converted to RGB when loaded. Use AutoYCbCr=0 to disable
  this behavior. \endverbatim
   * \ingroup format */
  ...
  \verbatim
      ...
      Attributes:
        ...
        PrimaryChromaticities  IMFLOAT (6)
        XPosition, YPosition IM_FLOAT (1)
        sRGBIntent IM_INT (1) [0: Perceptual, 1: Relative colorimetric, 2:
  Saturation, 3: Absolute colorimetric] TransparencyIndex IM_BYTE (1 or N)
        TransparentColor IM_BYTE (3)
        CalibrationName, CalibrationUnits (string)
        CalibrationLimits IM_INT (2)
        CalibrationEquation IM_BYTE (1)
  [0-Linear,1-Exponential,2-Arbitrary,3-HyperbolicSine)] CalibrationParam
  (string) [params separated by '\\n'] Title, Author, Description, Copyright,
  DateTime (string) Software, Disclaimer, Warning, Source, Comment, ... (string)
      ...
      Comments:
        ...
        pnggccrd.c - PNG_USE_PNGGCCRD
                     For Intel x86 CPU (Pentium-MMX or later) and GNU C
  compiler. Changed pngconf.h to use int instead of long in png_uint_32 and
  png_int_32. \endverbatim
   * \ingroup format */
  ...
  \verbatim
      ...
      Attributes:
        ScreenHeight, ScreenWidth IM_USHORT (1) screen size [default to the
  first image size] Interlaced IM_INT (1 | 0) default 0 Description (string)
  ...
  \verbatim
      ...
      Internally the components are always packed.
      Lines arranged from top down to bottom or bottom up to top. But are saved
  always as bottom up.
  ...
  \verbatim
      Data Types: Byte and UShort
      Color Spaces: Gray and RGB (Binary saved as Gray, MAP with fixed palette
  when reading only) Compressions: NONE - no compression  [default] RLE  - Run
  Lenght Encoding Only one image. Can have an alpha channel (only for IM_RGB)
      Internally the components are always packed.
  ...
  \verbatim
      ...
      Compressions:
        NONE - no compression
        RLE  - Run Lenght Encoding [default - since uncompressed PCX is not well
  supported] Only one image. No alpha channel. Internally the components are
  always packed. Internally the lines are arranged from top down to bottom.
  ...
  \verbatim
      ...
      No alpha channel.
      Internally the components are always packed.
      Internally the lines are arranged from bottom up to top or from top down
  to bottom.
  ...
  \verbatim
      ...
      Comments:
        ...
        For IM_MAP imagens, if the user does not specifies an alpha channel
          ...
          and the most repeated index with transparency will be the transparent
  index. Although any size and bpp can be used is recomended to use the typical
  configurations: 16x16, 32x32, 48x48, 64x64 or 96x96 2 colors, 16 colors or 256
  colors \endverbatim
   * \ingroup format */
  ...
  \verbatim
      ...
      Comments:
        ...
        Remeber that usually convolution operations use kernel size an odd
  number.
